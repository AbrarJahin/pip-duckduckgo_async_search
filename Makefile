.PHONY: help clear install update activate status clean delete run python build publish testpypi
...
# Create env if missing, else update it. Then install your project editable.
install:
	@echo ------------------------------------------------------------
...
# Force update (same as install, but explicit)
update: install
...
# Small demo run
run:
	@$(RUN) python -c "import asyncio; from DuckDuckGoAsyncSearch import top_n_result; \
async def main(): \
    items = await top_n_result('Capital of Bangladesh', n=5); \
    print('Results:', len(items)); \
    [print('-', i.title, i.url) for i in items]; \
asyncio.run(main())"
...
[os.remove(f) for f in os.listdir('.') if f.endswith('.egg-info') and os.path.isdir(f)]; \
print('Cleaned build artifacts (dist/, build/, cache).')"
